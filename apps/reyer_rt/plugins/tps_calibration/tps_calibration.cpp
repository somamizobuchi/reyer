...
class TpsCalibration : public RenderPluginBase<TpsCalibrationConfig> {
  public:
  ...
  protected:
    void onInit() override {
        tps_ = std::make_unique<TPSCalibration>();
        current_point_ = 0;
    ...
    void onShutdown() override { tps_.reset(); }

    void onRender() override {
        if (current_point_ >= 9) {
            // All points collected — fit model and push calibration to pipeline
            tps_->pushCalibrationPoints(collected_points_.data(),
                                        collected_points_.size());
            pushCalibration(std::move(tps_));
            current_point_ = 0;
            collected_points_.clear();
            is_calibrated_ = true;
            return;
  ...
  private:
    std::unique_ptr<TPSCalibration> tps_;
...
REYER_PLUGIN_ENTRY(reyer::plugin::TpsCalibration, "TPS Calibration",
                   "Soma Mizobuchi", "TPS Calibration task.", 1)
